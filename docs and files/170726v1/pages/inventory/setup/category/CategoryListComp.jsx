...
const CategoryListComp = ({ pageAuth, dataList, onEdit, onDelete }) => {
  const [globalFilter, setGlobalFilter] = useState(null);

  const export_columns = [
    { header: "Code", accessor: "mcatg_mcode" },
    { header: "Name", accessor: "mcatg_mname" },
    { header: "Active", accessor: "mcatg_actve" },
  ];
    ...
    return (
      <div className="flex flex-column">
        <span className="text-sm">
          <ActiveRowCell
            text={rowData.mcatg_mname}
            status={rowData.mcatg_actve}
          />
        </span>
  ...
  const action_BT = (rowData) => {
    let menuItems = [
      {
        label: rowData.mcatg_actve ? "Deactivate" : "Activate",
        icon: `pi ${
          rowData.mcatg_actve
    ...
    return (
      <div className="flex flex-column md:flex-row align-items-center justify-content-between gap-3">
        ...
        <div className="flex flex-wrap align-items-center gap-2 w-full md:w-auto">
          <CSVExport
            data={dataList}
            fileName={`category-${new Date().toISOString().slice(0, 10)}`}
