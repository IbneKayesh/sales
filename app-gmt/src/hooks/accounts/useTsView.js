...
const useTsView = () => {
  ...
  const [crTitle, setCrTitle] = useState("TS List");
  ...
  const loadBank = async () => {
    try {
      setIsBusy(true);
      const apiVersion = "/v7/";
      const apiEndPoint = "/data?key=tm_acmb;tm_acmp;tm_bank;tm_scam";
      const resp = await apiRequest(apiVersion, apiEndPoint, {});
      //console.log("resp", resp.tm_acmb.data);
      setDataList(resp.tm_acmb.data);
    } catch (error) {
    } finally {
      setIsBusy(false);
    }
  ...
  const onDelete = async (rowData) => {
    try {
      ...
      if (resp.tm_role.status) {
        await loadRoles();
        alert({
          message: `TS deleted successfully ${rowData.role_name}`,
          header: "Deleted",
        });
      } else {
        alert({
    ...
    try {
      ...
      if (formData.id) {
        ...
        if (resp.tm_role.status) {
          ...
          alert({
            message: `TS edited successfully ${formData.role_name}`,
            header: "Edited",
          });
          setCrTitle("TS List");
          setCrView("list");
        ...
        if (resp.tm_role.status) {
          ...
          alert({
            message: `TS saved successfully ${formData.role_name}`,
            header: "Saved",
          });
          setCrTitle("TS List");
          setCrView("list");
