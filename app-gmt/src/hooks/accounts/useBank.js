...
const useBank = () => {
  ...
  const [crTitle, setCrTitle] = useState("Bank List");
  ...
  const loadBank = async () => {
    try {
      setIsBusy(true);
      const apiVersion = "/v7/";
      const apiEndPoint = "/data?key=tm_bank";
      const resp = await apiRequest(apiVersion, apiEndPoint, {});
      //console.log("resp", resp.tm_bank.data);
      setDataList(resp.tm_bank.data);
    } catch (error) {
    } finally {
      setIsBusy(false);
    }
