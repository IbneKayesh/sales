...
const useUnits = () => {
  ...
  const [crTitle, setCrTitle] = useState("Unit List");
  ...
  const loadUnits = async () => {
    try {
      setIsBusy(true);
      const apiVersion = "/v7/";
      const apiEndPoint = "/data?key=tm_unit";
      const resp = await apiRequest(apiVersion, apiEndPoint, {});
      //console.log("resp", resp.tm_unit.data);
      setDataList(resp.tm_unit.data);
    } catch (error) {
    } finally {
      setIsBusy(false);
    }
  ...
  const onDelete = async (rowData) => {
    try {
      ...
      if (resp.tm_role.status) {
        await loadRoles();
        alert({
          message: `Unit deleted successfully ${rowData.role_name}`,
          header: "Deleted",
        });
      } else {
        alert({
  ...
  const handleBackClick = () => {
    setCrTitle("Unit List");
    setCrView("list");
    setFormData(dataModel);
  };

  const handleSearchClick = () => {
    setCrTitle("Search Unit");
    setCrView("list");
    ...
    try {
      ...
      if (formData.id) {
        ...
        if (resp.tm_role.status) {
          ...
          alert({
            message: `Unit edited successfully ${formData.role_name}`,
            header: "Edited",
          });
          setCrTitle("Unit List");
          setCrView("list");
        ...
        if (resp.tm_role.status) {
          ...
          alert({
            message: `Unit saved successfully ${formData.role_name}`,
            header: "Saved",
          });
          setCrTitle("Unit List");
          setCrView("list");
