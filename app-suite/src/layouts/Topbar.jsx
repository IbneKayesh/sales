...
export default function Topbar({ className = '', ...rest }) {
  ...
  const notifications = [
    ...
    { id: 2, type: 'warning', icon: <IconBox size={16} />, title: 'Low stock alert', message: 'Office supplies running low — 3 items below threshold', time: '18 min ago', read: false },
    { id: 3, type: 'info', icon: <IconActivity size={16} />, title: 'New user registered', message: 'Leo Martinez created an account', time: '1 hr ago', read: false },
    { id: 4, type: 'danger', icon: <IconClose size={16} />, title: 'Payment failed', message: 'Google Ads campaign payment declined — update billing', time: '3 hr ago', read: true },
    { id: 5, type: 'success', icon: <IconCheck size={16} />, title: 'Report ready', message: 'Q3 Financial Summary is available for download', time: '5 hr ago', read: true },
  ]
  ...
  const hasAvatar = !!(user?.avatar && (user.avatar.startsWith('http') || user.avatar.startsWith('data:')))
    ...
    <header className={`topbar${className ? ' ' + className : ''}`} {...rest}>
      ...
      <div className="topbar__right">
          ...
          {notifOpen && (
            <div className="topbar__notif-panel">
              <div className="topbar__notif-header">
                <span className="topbar__notif-header-title">Notifications</span>
                <button
                  type="button"
              ...
              <div className="topbar__notif-footer">
                <button type="button" className="topbar__notif-view-all" onClick={() => { setNotifOpen(false); navigate('/notifications') }}>
        ...
        <div className="topbar__profile-wrap" ref={profileRef}>
          ...
          >
            {hasAvatar ? (
              <img className="topbar__avatar-img" src={user.avatar} alt={user.name || 'User'} />
            ) : (
              <span className="topbar__avatar-initials">{initials}</span>
            )}
            <span className={`topbar__avatar-status ${user?.status === 'active' ? 'topbar__avatar-status--online' : ''}`} />
          </button>
            ...
            <div className="topbar__profile-menu">
              <div className="topbar__profile-header">
                <div className="topbar__profile-header-left">
                  {hasAvatar ? (
                    <img className="topbar__profile-avatar-img" src={user.avatar} alt="" />
                  ) : (
                    <span className="topbar__profile-avatar-letter">{initials}</span>
                  )}
                </div>
                <div className="topbar__profile-header-text">
                  <span className="topbar__profile-name">{user?.name || 'User'}</span>
                  <span className="topbar__profile-email">{user?.email || ''}</span>
                  {user?.role && (
                    <span className="topbar__profile-role">{user.role.charAt(0).toUpperCase() + user.role.slice(1)}</span>
                  )}
                </div>
              </div>
              <div className="topbar__profile-divider" />
              <div className="topbar__profile-actions">
                <button type="button" className="topbar__profile-action" onClick={() => navigate('/users')}>
                  <IconUsers size={16} />
                  Profile settings
                </button>
                <button type="button" className="topbar__profile-action" onClick={() => navigate('/settings')}>
                  <IconEdit size={16} />
                  Account settings
                </button>
