...
export function validateModel(data, model, options = {}) {
  ...
  for (const [fieldName, def] of Object.entries(fields)) {
    ...
    const value = trimStrings && typeof rawValue === 'string' ? rawValue.trim() : rawValue;

    // ── Required check ──────────────────────────────────────────────────
    if (def.required) {
      if (value === undefined || value === null || value === '') {
        errors[fieldName] = def.errorMessage || `${def.label || fieldName} is required`;
        continue;
      }
    } else {
    ...
    switch (resolvedType) {
      ...
      case 'string': {
        ...
        if (def.minLength != null && strVal.length < def.minLength) {
          errors[fieldName] = `Minimum ${def.minLength} character${def.minLength > 1 ? 's' : ''} required`;
          break;
        }

        if (def.maxLength != null && strVal.length > def.maxLength) {
          errors[fieldName] = `Maximum ${def.maxLength} character${def.maxLength > 1 ? 's' : ''}`;
          break;
        }
        ...
        if (pattern && !new RegExp(pattern).test(strVal)) {
          errors[fieldName] = def.patternMessage || getDefaultPatternMessage(def.type);
        }
        break;
      }
      ...
      case 'number': {
        ...
        if (def.type === 'decimal' && def.precision != null) {
          ...
          if (decimalPlaces > def.precision) {
            errors[fieldName] = `At most ${def.precision} decimal place${def.precision > 1 ? 's' : ''} allowed`;
            break;
          }
        }
        break;
      ...
      case 'array': {
        ...
        if (def.minItems != null && value.length < def.minItems) {
          errors[fieldName] = `At least ${def.minItems} item${def.minItems > 1 ? 's' : ''} required`;
          break;
        }

        if (def.maxItems != null && value.length > def.maxItems) {
          errors[fieldName] = `At most ${def.maxItems} item${def.maxItems > 1 ? 's' : ''} allowed`;
          break;
        }

        // Resolve item model (inline itemModel or via itemModelRef)
        const itemModel = def.itemModel || (def.itemModelRef && models[def.itemModelRef]);
        ...
        if (itemModel && value.length > 0) {
          value.forEach((item, idx) => {
            for (const [itemField, itemDef] of Object.entries(itemModel)) {
              ...
              const itemVal = trimStrings && typeof itemRaw === 'string' ? itemRaw.trim() : itemRaw;
                ...
                if (itemVal === undefined || itemVal === null || itemVal === '') {
                  if (!errors[`${fieldName}[${idx}].${itemField}`]) {
                    errors[`${fieldName}[${idx}].${itemField}`] =
                      itemDef.errorMessage || `${itemDef.label || itemField} is required`;
                  }
                  continue;
                }
              ...
              if (itemResolved.resolvedType === 'string') {
                ...
                if (itemDef.enum && !itemDef.enum.includes(strVal)) {
                  errors[`${fieldName}[${idx}].${itemField}`] = `Must be one of: ${itemDef.enum.join(', ')}`;
                } else if (itemDef.minLength != null && strVal.length < itemDef.minLength) {
                  errors[`${fieldName}[${idx}].${itemField}`] = `Minimum ${itemDef.minLength} characters`;
                } else if (itemDef.maxLength != null && strVal.length > itemDef.maxLength) {
                  errors[`${fieldName}[${idx}].${itemField}`] = `Maximum ${itemDef.maxLength} characters`;
                }
              } else if (itemResolved.resolvedType === 'number') {
                const numVal = Number(itemVal);
                if (isNaN(numVal)) {
                  errors[`${fieldName}[${idx}].${itemField}`] = itemDef.errorMessage || 'Must be a valid number';
                } else if (itemDef.type === 'integer' && !Number.isInteger(numVal)) {
                  errors[`${fieldName}[${idx}].${itemField}`] = 'Must be a whole number';
                } else if (itemDef.min != null && numVal < itemDef.min) {
                  errors[`${fieldName}[${idx}].${itemField}`] = `Must be at least ${itemDef.min}`;
                } else if (itemDef.max != null && numVal > itemDef.max) {
                  errors[`${fieldName}[${idx}].${itemField}`] = `Must be at most ${itemDef.max}`;
                }
              }
            }
          });
