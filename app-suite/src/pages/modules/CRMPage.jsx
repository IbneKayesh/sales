...
export default function CRMPage() {
  ...
  return (
    <PageShell
      title={isListView ? 'CRM' : (view === 'add' ? 'Add Contact' : 'Edit Contact')}
      subtitle={isListView ? 'Customer relationship management' : undefined}
      compact
    >
      <PageShell.Actions>
        {isListView ? (
      ...
      {isListView && (
        <PageShell.Stats>
          <PageShell.Stat label="Total Contacts" value={contacts.length} />
          <PageShell.Stat label="Customers" value={contacts.filter(c => c.stage === 'Customer').length} />
          <PageShell.Stat label="Leads" value={contacts.filter(c => c.stage === 'Lead').length} />
          <PageShell.Stat label="Prospects" value={contacts.filter(c => c.stage === 'Prospect').length} />
        </PageShell.Stats>
      )}
      ...
      <PageShell.Body>
        {isListView ? (
          ...
          <form onSubmit={handleSubmit} id="crm-form" style={{ display:'flex', flexDirection:'column', gap:20, maxWidth:600 }}>
