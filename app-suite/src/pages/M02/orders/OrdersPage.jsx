...
import useOrders from "@/hooks/M02/useOrders";
  ...
  return (
    <div className="page-wrap">
      <PageCard>
        <PageCardHeader>
          <PageCardTitle
            title={pgView === "SYS_VW_LST_1" ? "Orders" : "Order Entry"}
            subtitle={
              pgView === "SYS_VW_LST_1"
                ? listData.length + " Orders"
                : formData?.mrrdm_trnno || "New Order"
            }
          />
          <PageCardActions>
            {pgView === "SYS_VW_LST_1" && (
        ...
        <PageCardBody>
          {pgView === "SYS_VW_LST_1" && (
            <OrderList
              cfColumns={tcVisibleItem.filter(
                (f) => f.tabcl_table === "SYS_ORDER_LIST",
              )}
              listData={listData}
              onEdit={handleEdit}
          ...
          {pgView === "SYS_VW_FRM_1" && listDataItem.length > 0 && (
            <ItemList
              cfColumns={tcVisibleItem.filter(
                (f) => f.tabcl_table === "SYS_FORMS_ITEMS",
              )}
              readOnly={readOnly}
