...
import t_units from "@/models/setup/t_units.json";
...
export const useUnits = () => {
  ...
  const handleSaveUnit = async (e) => {
    ...
    console.log("handleSaveUnit: " + JSON.stringify(newErrors));
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        if (formDataUnit.unit_id) {
          // Edit existing
          const updatedUnit = await unitsAPI.update(formDataUnit.unit_id, formDataUnit);
          ...
          // Add new
          const newUnit = await unitsAPI.create(formDataUnit);
