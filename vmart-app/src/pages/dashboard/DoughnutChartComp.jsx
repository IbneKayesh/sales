...
const DoughnutChartComp = ({ title, data, isLoading }) => {
    ...
    <div className="surface-card p-3 border-round shadow-2 h-full flex flex-column" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
      ...
      <div className="flex-grow-1 flex flex-column align-items-center justify-content-center">
        <div className="relative" style={{ width: '180px', height: '180px' }}>
          ...
          <div className="absolute top-0 left-0 w-full h-full flex align-items-center justify-content-center pointer-events-none">
            <div className="flex flex-column align-items-center justify-content-center" style={{ width: '65%', height: '65%', borderRadius: '50%', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-light)' }}>
                {hovered !== null ? (
                    <>
                        <span className="text-xl font-bold" style={{ color: colors[hovered % colors.length] }}>{Math.round((values[hovered]/total)*100)}%</span>
                        <span className="text-xs text-muted font-medium text-center px-2">{labels[hovered]}</span>
                    </>
                ) : (
                    <span className="text-xs text-muted">Total: {total.toLocaleString()}</span>
                )}
            </div>
          </div>
        </div>
        <div className="mt-4 flex flex-wrap justify-content-center gap-2">
          {labels.map((label, idx) => (
            <div key={idx} className="flex align-items-center p-1 px-2 border-round" style={{ backgroundColor: hovered === idx ? 'var(--hover-bg)' : 'transparent' }}>
              ...
              <span className="text-xs" style={{ color: hovered === idx ? 'var(--text-main)' : 'var(--text-muted)' }}>{label}</span>
            </div>
          ))}
        </div>
      </div>
