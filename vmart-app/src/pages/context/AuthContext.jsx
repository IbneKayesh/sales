...
export function AuthProvider({ children }) {
  ...
  const registerUser = useCallback((profile) => {
    ...
    const entry = {
      ...
      securityQuestion: profile.securityQuestion || registry[existing]?.securityQuestion || "",
      securityAnswer: profile.securityAnswer || registry[existing]?.securityAnswer || "",
    };
    if (existing !== -1) {
      registry[existing] = { ...registry[existing], ...entry, password: entry.password || registry[existing].password };
    } else {
      registry.push(entry);
    }
  ...
  const verifyPassword = useCallback((name, role, password) => {
    ...
    if (user.password !== password) return { valid: false, reason: "Incorrect password" };
    return { valid: true, data: user };
  }, []);

  const loginAsCustomer = useCallback((profile) => {
    const authData = {
      name: profile.name,
      contact: profile.contact || "",
      address: profile.address || "",
  ...
  const loginAsShop = useCallback((profile) => {
    const authData = {
      name: profile.name,
  ...
  const verifySecurityAnswer = useCallback((name, role, answer) => {
    ...
    const user = registry.find((u) => u.name === name && u.role === role);
    if (!user) return { valid: false, reason: "User not found" };
    if (!user.securityAnswer) return { valid: false, reason: "No security question set" };
  ...
  const value = useMemo(() => ({
    ...
    updateProfile,
    changePassword,
  }), [user, isAuthenticated, hasPassword, lookupUser, verifyPassword, registerUser, loginAsCustomer, loginAsShop, logout, updateProfile, changePassword]);
