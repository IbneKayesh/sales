...
const PartyListComp = ({ pageAuth, dataList, onEdit, onDelete }) => {
  const [globalFilter, setGlobalFilter] = useState(null);

  const export_columns = [
    { header: "Type", accessor: "party_ptype" },
    { header: "Code", accessor: "party_pcode" },
    { header: "Name", accessor: "party_pname" },
    { header: "Opening", accessor: "party_opbal" },
    { header: "Active", accessor: "party_actve" },
  ];

  const party_pname_BT = (rowData) => {
    return (
      <div className="flex flex-column">
        <span className="text-sm">
          <ActiveRowCell
            text={rowData.party_pname}
            status={rowData.party_actve}
          />
        </span>
  ...
  const action_BT = (rowData) => {
    let menuItems = [
      {
        label: rowData.party_actve ? "Deactivate" : "Activate",
        icon: `pi ${
          rowData.party_actve
    ...
    return (
      <div className="flex flex-column md:flex-row align-items-center justify-content-between gap-3">
        ...
        <div className="flex flex-wrap align-items-center gap-2 w-full md:w-auto">
          <CSVExport
            data={dataList}
            fileName={`party-${new Date().toISOString().slice(0, 10)}`}
