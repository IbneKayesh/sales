...
const UnitsListComp = ({ pageAuth, dataList, onEdit, onDelete }) => {
  const [globalFilter, setGlobalFilter] = useState(null);

  const export_columns = [
    { header: "Group", accessor: "units_untgr" },
    { header: "Code", accessor: "units_ucode" },
    { header: "Name", accessor: "units_uname" },
    { header: "Active", accessor: "units_actve" },
  ];

  const units_uname_BT = (rowData) => {
    return (
      <div className="flex flex-column">
        <span className="text-sm">
          <ActiveRowCell
            text={rowData.units_uname}
            status={rowData.units_actve}
          />
        </span>
  ...
  const action_BT = (rowData) => {
    let menuItems = [
      {
        label: rowData.units_actve ? "Deactivate" : "Activate",
        icon: `pi ${
          rowData.units_actve
  ...
  return dataList.length > 0 ? (
    ...
    >
      ...
      <Column
        ...
        body={units_uname_BT}
      />
      <Column field="units_untgr" header="Group" sortable />
