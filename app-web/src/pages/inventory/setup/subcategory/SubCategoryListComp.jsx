...
const SubCategoryListComp = ({ pageAuth, dataList, onEdit, onDelete }) => {
  const [globalFilter, setGlobalFilter] = useState(null);

  const export_columns = [
    { header: "Code", accessor: "scatg_scode" },
    { header: "Name", accessor: "scatg_sname" },
    { header: "Active", accessor: "scatg_actve" },
  ];
    ...
    return (
      <div className="flex flex-column">
        <span className="text-sm">
          <ActiveRowCell
            text={rowData.scatg_sname}
            status={rowData.scatg_actve}
          />
        </span>
  ...
  const action_BT = (rowData) => {
    let menuItems = [
      {
        label: rowData.scatg_actve ? "Deactivate" : "Activate",
        icon: `pi ${
          rowData.scatg_actve
