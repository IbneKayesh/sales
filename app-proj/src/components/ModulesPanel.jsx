...
export default function ModulesPanel({
  ...
  const expandedModules = expandedModuleIds || new Set();
  const setExpandedModules = setExpandedModuleIds;
  const expandedSubmodules = expandedSubmoduleIds || new Set();
  ...
  const beginAdd = (type, parent = {}) => {
    const nextDraft = {
      module: {
        ...blankModule,
        project_id: parent.project_id || (projects.length > 0 ? projects[0].id : ""),
      },
      submodule: {
        ...blankSubmodule,
    ...
    <div className="grid-workspace fade-in">
      ...
      <div className="card grid-card">
        <div className="card-header">
          <div>
            <span className="card-title">Modules scoping</span>
            <span className="card-subtitle">Manage module scoping, timelines, status tracking, and database bindings.</span>
          </div>
          <button
            className="btn btn-primary btn-xs"
            onClick={() => beginAdd("module")}
                ...
                <tr className="empty-grid-row">
                  <td colSpan={8}>
                    <div className="empty-row-action">
                      <span>No modules mapped to any project yet.</span>
                      <button
                        className="btn btn-primary btn-xs"
                        onClick={() => beginAdd("module")}
