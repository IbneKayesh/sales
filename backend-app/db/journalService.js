...
async function buildJournalScripts({
  ...
  if (acprd.length > 1) {
    throw new Error("Multiple active accounting periods found. Please select one.");
  }
  ...
  scripts.push({
    ...
    params: [
      masterId, user_c, user_b, dpart, fsyar_id, acprd_id,
      crncy.crncy_tcrnc, trtyp, trnNo, trdat, refno, narrt,
      drval, crval, crncy.crncy_exrat, "Posted", user_s, user_s,
    ],
  ...
  for (const det of details) {
    scripts.push({
      ...
      params: [
        uuidv4(), user_c, user_b, dpart, masterId, det.chtac,
        det.party, det.drval || 0, det.crval || 0, det.descr,
        det.sorce, det.refid, det.rtype || "MASTER", line++,
        user_s, user_s,
      ],
