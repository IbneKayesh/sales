...
router.get("/details/:master_id", async (req, res) => {
  try {
    ...
    LEFT JOIN contacts c ON pom.contact_id = c.contact_id
    WHERE pom.master_id = ?`;
    const rows = await dbGet(sql, [master_id]);
    res.json(rows);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
...
router.post("/update", async (req, res) => {
  try {
    ...
    const sql = `UPDATE po_master SET shop_id = ?, contact_id = ?, order_type = ?, order_no = ?, order_date = ?, order_note = ?, order_amount = ?, discount_amount = ?, vat_amount = ?, is_vat_payable = ?, include_cost = ?, exclude_cost = ?, total_amount = ?, payable_amount = ?, paid_amount = ?, due_amount = ?, is_paid = ?, is_posted = ?, is_returned = ?, is_closed = ? WHERE master_id = ?`;
