...
router.post("/", async (req, res) => {
  try {
    ...
    const rows = await dbGetAll(
      sql,
      params,
      `Get receivables for ${rcvbl_users}`,
    );
    res.json({
      success: true,
  ...
  try {
    ...
    if (rcvbl_srcnm === "Sales Booking") {
      scripts.push({
        ...
        label: `Update Sales Booking ${rcvbl_refno}`,
      });
      ...
      scripts.push({
        ...
        END WHERE id = ?`,
        params: [rcvbl_refid],
        label: `Update Sales Booking Status ${rcvbl_refno}`,
      });
    }
...
// receivable details
router.post("/receivable-details", async (req, res) => {
  try {
    const {
      rcvbl_users,
      rcvbl_bsins,
...
router.post("/receivable-summary", async (req, res) => {
  try {
    ...
    //database action
    let sql = `SELECT cnt.cntct_cntnm, pbl.rcvbl_refno, pbl.rcvbl_srcnm, SUM(pbl.rcvbl_dbamt) AS rcvbl_dbamt, SUM(pbl.rcvbl_cramt) AS rcvbl_cramt
    FROM tmtb_rcvbl pbl
    LEFT JOIN tmcb_cntct cnt ON pbl.rcvbl_cntct = cnt.id
    WHERE pbl.rcvbl_bsins = ?
    `;
    let params = [rcvbl_bsins];

    // Optional filters
    if (rcvbl_refno) {
      sql += ` AND pbl.rcvbl_refno LIKE ?`;
      params.push(`%${rcvbl_refno}%`);
    }
    if (rcvbl_cntct) {
    ...
    if (search_option) {
      switch (search_option) {
        case "last_3_days":
          sql += ` AND pbl.rcvbl_trdat >= DATE_SUB(CURRENT_DATE(), INTERVAL 3 DAY)`;
          break;
        case "last_7_days":
          sql += ` AND pbl.rcvbl_trdat >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)`;
          break;
        default:
          sql += ``;
          break;
    ...
    sql += ` GROUP BY cnt.cntct_cntnm, pbl.rcvbl_refno, pbl.rcvbl_srcnm`;
    sql += ` ORDER BY pbl.rcvbl_refno`;
