...
router.post("/", async (req, res) => {
  try {
    ...
    const rows = await dbGetAll(sql, params, `Get leave entitlement for ${lvntl_users}`);
    res.json({
      success: true,
...
router.post("/create", async (req, res) => {
  try {
    const { id, lvntl_users, lvntl_bsins, lvntl_yerid, lvntl_atnst, lvntl_nmbol, suser_id } = req.body;

    // Validate input
    if (!id || !lvntl_users || !lvntl_bsins || !lvntl_yerid || !lvntl_atnst || lvntl_nmbol === undefined) {
      return res.json({
        success: false,
    ...
    const params = [id, lvntl_users, lvntl_bsins, lvntl_yerid, lvntl_atnst, lvntl_nmbol, suser_id, suser_id];
...
router.post("/get-all-active", async (req, res) => {
  try {
    ...
    const rows = await dbGetAll(sql, params, `Get active leave entitlement for ${lvntl_users}`);
    res.json({
      success: true,
